...
- Since the npm `dependencies` hook does not fire when the dependencies of `other-repo` change (see npm bug [#8984](https://github.com/npm/cli/issues/8984)), Nudeps on `other-repo` will run `npm run dependencies --if-present` in each repo that depends on it, to trigger nudeps there.
