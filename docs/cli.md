...
Install Nudeps into a project: adds it to your `devDependencies` (setting `type: "module"` while it is there), adds the `dependencies` and `prepare` npm lifecycle scripts that keep your import map up to date, then runs Nudeps once to initialize.
