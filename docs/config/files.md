...
- `package-lock.json` and `pnpm-lock.json` files at the top level of any package
...
Your entries come after the built-in defaults, so a `copy` glob reverses a default and an `ignore` glob adds to it:

- To ship a package's lockfile anyway: `ignore: { copy: "package-lock.json" }`.
- To skip readmes, which are copied by default: `ignore: "{readme,README}.*"`.
